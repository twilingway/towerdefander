...
Server SHALL принимать latency pong от display и controller только после strict protocol/schema и
...
simulation tick, fire/shield state, cooldown или energy. Valid duplicate, late, unknown либо чужой
probeId SHALL молча игнорироваться без изменения текущего sample; malformed payload SHALL получать
actor-only `invalid_message`, а protocol mismatch — actor-only `protocol_mismatch`.
...
- **THEN** server не добавляет второй sample и не меняет gameplay state

#### Scenario: Устаревший pong

- **WHEN** client отправляет latency pong с protocolVersion 6
