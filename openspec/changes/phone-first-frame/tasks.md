...
- [ ] 1.1 `balance.ts`: `CAMERA_VIEW_ASPECT = 9/19.5`, `cameraViewHeight`, `legacyFrameWidth`,
      пределы ширины 975…5363; `scanRadiusScreens` → `scanRadiusCells`; `BALANCE_FILE_VERSION` 58
- [ ] 1.2 `index.ts`: `PROTOCOL_VERSION` 72; комментарии о пропорции; тесты протокола
- [ ] 1.3 game-core: ширина кадра 3047, `ARENA_SCAN_RADIUS_CELLS = 1`
...
- [ ] 2.1 Миграция 58: ширины из файла × 19.5/16 с округлением, только для файлов старше 58;
      `scanRadiusScreens` → `scanRadiusCells`; тест на файле 57 и на сиде
- [ ] 2.2 `crewPolicy.mjs`: `frameRadius`, `sourceIsSpent`, догадка о стрелке через
...
- [ ] 3.1 Подсказки кадра 19.5:9 на «Директоре»; поле и подсказка скана в клетках на «Арене»; тексты
      «Автопилота» и «Руля», где они говорят о сторонах кадра

Проверка: `pnpm --filter @spaceship-defender/admin exec vitest run`.

## 4. Дисплей: кадр

- [ ] 4.1 Камера и превью под новую пропорцию; ширина превью 2681; подпись ползунка превью
- [ ] 4.2 Звук через `legacyFrameWidth`; небо: запас картинки и число звёзд от кадра
- [ ] 4.3 `useWorldFrame`: переменные прямоугольника кадра на оболочке боя; юнит-тест расчёта

Проверка: `pnpm --filter @spaceship-defender/display exec vitest run`, `pnpm typecheck`.

## 5. Дисплей: HUD

- [ ] 5.1 Рамка сведений, таймер, рамка состояния и радар у кадра мира; верхний ряд в полосе, где
      она есть
- [ ] 5.2 Стики и кнопки у краёв экрана с safe area; нажимаемое не меньше 44 px и не ближе 16 px к
      краю
...
- [ ] 6.3 `spaceshipViewModel.test.ts`: размеры кадра через пропорцию, а не `9 / 16`
- [ ] 6.4 `docs/GAME_DESIGN_DOCUMENT.md` и `CLAUDE.md`: кадр и версии
