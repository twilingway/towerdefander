...
- [x] 2.2 Extend Colyseus state with assigned sectors and the public game snapshot; verify schema
      mapping with server tests.
- [x] 2.3 Start exactly one room-owned fixed-step scheduler, advance game-core, and finish the room
      on victory/defeat; verify lifecycle tests with fake timers.
- [x] 2.4 Route repair/upgrade through room/player identity, phase, ownership, funds, and a
      room-wide accepted/rejected action-result journal; verify atomicity and replay tests.
- [x] 2.5 Preserve active-game identity and snapshot across controller/display reconnect; verify
      room and network smoke tests.

## 3. Browser clients

- [x] 3.1 Replace display signal count with a readable two-sector battlefield, treasury, health,
      defense, enemies, and result view; verify component tests and production build.
- [x] 3.2 Replace controller signal action with own-sector repair/upgrade controls and
      server-confirmed feedback; verify component and interaction tests.
- [x] 3.3 Keep lobby, protocol mismatch, reconnect, unavailable-action, and insufficient-funds
      errors usable on phone and desktop layouts; verify browser E2E.

## 4. Integrated verification

- [x] 4.1 Extend network smoke through room start, both action types, duplicate delivery, reconnect,
      and terminal result.
- [x] 4.2 Extend Playwright E2E so two isolated controllers complete a short deterministic match.
- [x] 4.3 Run `pnpm check` and `pnpm spec:validate`, complete reviewer-agent reconciliation, and
      archive the change.
