...
- [x] 7.1 Оформить change: proposal, design с отклонёнными альтернативами, дельты на
      `primitive-top-down-battlefield`, `balance-admin-console`, `codebase-structure`. Проверка:
      `pnpm spec:validate`.
- [ ] 7.2 Полный гейт: `pnpm check`.
- [ ] 7.3 Замер кадра до и после: `node scripts/profile-display.mjs --cpu=4 --wave=8` и
      `node scripts/bench-panels.mjs` — в кадре появился новый анимированный спрайт.
- [ ] 7.4 Приёмка глазами: факел в полёте, а не на припаркованном корабле; каталог в консоли на
      `/effects`.
