...
- [x] 7.2 Замер кадра снят: `node scripts/profile-display.mjs --cpu=4 --wave=8` при
      `BENCH_URL=http://127.0.0.1:5173/` — 42.3% кадра простой, 25.6% `(program)`. Ни одна функция
      эффектов в профиле не всплывает: самое тяжёлое из нашего это рисование радара (1.0%, 10.2
      мс/с) и `drawCombatRadar` (0.7%, 7.2 мс/с), `updateScene` 0.4% (4.0 мс/с). Два новых источника
      вспышек кадр не съели.
