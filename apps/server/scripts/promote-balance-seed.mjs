...
import { migrateBalanceDocument } from "../../../packages/balance-core/src/migrations.ts";

const defaultSource = fileURLToPath(new URL("../data/balance.json", import.meta.url));
const defaultSeed = fileURLToPath(
...
/**
 * The revision a bundler can read. The text file beside the seed is what the
 * release script reads with a shell; a client build cannot read a loose file,
 * so the same number is mirrored into a generated module of the package.
 *
 * Only the package's own seed has such a module. A promotion aimed elsewhere
 * with `--out` -- an export, a test, a copy for comparison -- writes the two
 * files it was asked for and nothing else, rather than reaching back into a
 * source tree it was not pointed at.
 */
function revisionModulePathFor(seed) {
  return seed === defaultSeed ? resolve(seed, "../../src/seedRevision.ts") : undefined;
}

function revisionModuleSource(revision) {
  return [
    "/**",
    ...
    " * carrying.",
    " */",
    `export const SEED_REVISION = ${String(revision)};`,
    ""
  ].join("\n");
}
...
const revisionModulePath = revisionModulePathFor(seedPath);
if (revisionModulePath !== undefined) {
  await writeFile(
    revisionModulePath,
    await formatSeed(revisionModuleSource(nextRevision), revisionModulePath),
    "utf8"
  );
}
...
console.log(
  `Promoted ${sourcePath}\n` +
    ...
    (revisionModulePath === undefined ? "" : `  revision module ${revisionModulePath}\n`) +
    "Commit the written files; the next release delivers them to the production volume."
);
