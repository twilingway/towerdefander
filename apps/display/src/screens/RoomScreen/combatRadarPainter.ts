...
export function drawCombatRadar(
  ...
  if (game.arenaZones.length === 0) {
    ...
    // The shield's state is said on the status frame, in the bar's own caption,
    // so the dial leaves the word out rather than saying it twice.
  }
  ...
  context.fillStyle = "#bdfaff";
  // Said here rather than inherited: a dial that left the alignment to whatever
  // was drawn before it drew the speed from the plate's middle rightwards.
