...
Полный список — маршруты дисплея, вкладки консоли и отладочные флаги — `pnpm dev` печатает в
терминале при запуске. Вкладки он читает из `apps/admin/src/screens/registry.tsx`, поэтому список не
устаревает от того, что кто-то добавил экран. Отладочные флаги дисплея намеренно остаются
query-параметрами, а не маршрутами, и ни одна страница на них не ссылается: `?diag` — приборы,
`?tanks` — танковый руль, `?dpr=<n>` — плотность пикселей, `?preview=1` — кадр камеры без комнаты,
`?wave=<n>` — старт с поздней волны (нужен `ALLOW_START_WAVE`), `?ship=<id>` — корпус.

Переменные окружения: `.env.example` — шаблон, реальные значения кладутся в `.env.local` (в git не
попадает). Сервер читает **только** `.env.local`, явно, из `scripts/run-dev.mjs`; клиентам vite
отдаёт из корня репозитория то, что начинается на `VITE_`. Пароли `/stats/rooms` и `/admin/*`
локально не нужны — с петлевого адреса пускают без них (`apps/server/src/stats/access.ts`); на проде
они обязательны и живут в `.env.production` на хосте.
...
| Команда                                            | Что делает                                                                       |
| -------------------------------------------------- | -------------------------------------------------------------------------------- |
| `pnpm watch:bots`                                  | Окно Chrome, комнату ведёт её собственный экипаж: смотреть, как играет бот       |
