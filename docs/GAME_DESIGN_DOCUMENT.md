...
- Версия документа: 2026-08-26
- Текущий сетевой контракт: protocol v17
...
- камера кадрирует настраиваемый участок мира: `cameraViewWidth` из активного пресета баланса задаёт
  минимальную ширину видимой области (по умолчанию `2200` мировых единиц, высота — `9/16` от
  ширины), а на другом соотношении сторон камера расширяет видимую область по одной оси без
  растяжения мира;
...
Тип врага — это идентификатор записи в каталоге баланса, а не фиксированный enum: пять встроенных
архетипов правятся и дополняются из консоли баланса без пересборки сервера. Запись несёт HP, радиус,
скорость, дистанцию удержания, список орудий, силуэт с цветами, стоимость спавна, волну
разблокировки и награды.

| Архетип     |  HP | Скорость | Дистанция | Орудие                       | Дальность огня | Цена | Волна | Очки / кредиты |
| ----------- | --: | -------: | --------: | ---------------------------- | -------------: | ---: | ----: | -------------- |
| Перехватчик |  22 |      260 |       320 | пуля 4 урона каждые 0,6 с    |            600 |    1 |     1 | 12 / 1         |
| Ганшип      |  50 |      150 |       650 | пуля 10 урона каждые 1,5 с   |           1200 |    2 |     1 | 25 / 2         |
| Снайпер     |  70 |       70 |      1400 | пуля 35 урона каждые 5 с     |           3000 |    3 |     5 | 30 / 3         |
| Ракетоносец | 110 |       95 |       900 | ракета 30 урона каждые 3,5 с |           1700 |    4 |     3 | 25 / 4         |
| Босс        | 900 |       60 |       700 | залп трёх ракет каждые 3 с   |           1600 |   20 |    10 | 250 / 30       |
...
- **дальность огня** — с какого удаления орудие открывает огонь; дальше ствол молчит и держит заряд,
  поэтому стреляет в тот же тик, когда корабль вошёл в дальность;
- у архетипа может быть до четырёх орудий, каждое со своим cooldown, залпом и разбросом;
- босс приходит только на boss-волнах, показывает HP-бар над корпусом и удерживает волну до своей
  зачистки.

**Роли в бою:** перехватчики создают ближнее давление роем, ганшипы дают базовый перекрёстный огонь,
снайпер бьёт редко и больно с дистанции, ракетоносец заставляет gunner выбирать между уроном и
перехватом, босс задаёт пик волны.

Дальность огня и кадр камеры связаны: при кадре `2200` гарантированно виден радиус около `620`
мировых единиц, поэтому всё, что стреляет дальше, открывает огонь из-за края экрана. Консоль баланса
показывает это соотношение рядом с каждым орудием.

### 9.2. Asteroid — реализованная опасность окружения
...
### Реализовано: явная таблица поверх директора

Кампания гибридная. Явная таблица волн задаёт точный состав первых `N` волн, а всё, что дальше,
набирает процедурный директор.

Запись группы в таблице несёт тип, количество, интервал спавна внутри группы, набор секторов
появления и необязательные переопределения множителей HP и темпа. Секторы задаются как стороны
арены, поэтому волну можно завести с одного направления или развести по нескольким. Встроенный
пресет поставляется с пустой таблицей: без правок в консоли всю кампанию ведёт директор.

Директор:

- начальный wave budget `5`, прирост `+2` за волну, cap `120`;
- стоимость spawn берётся из каталога: asteroid и перехватчик `1`, ганшип `2`, снайпер `3`,
  ракетоносец `4`, босс `20`;
- interval между wave spawns `0,6 с`;
- enemy HP `+12%` за волну, cap `×8`; темп атак `+5%` за волну, cap `×3`;
...
- score: enemy ship `12`–`250` по каталогу, missile `5`, asteroid `10`; projectile kill и shield
  interception дают одинаковую однократную награду;
- при `HP=0` simulation замораживает финальное состояние и публикует `result/defeat`.

Score является нетратимым показателем результата run. Общие credits экипажа начисляются отдельно и
берутся из каталога архетипов: wave asteroid `1`, перехватчик `1`, ганшип `2`, снайпер `3`,
ракетоносец `4`, босс `30`; ambient asteroid и missile дают `0` credits, чтобы исключить бесконечный
фарм.
...
- protocol v17 использует строгие schemas и публикует wave countdown, economy, team votes, каталог
  визуалов врагов и кадр камеры;
...
Баланс живёт в файле пресетов версии `7` (`BALANCE_PRESET_PATH`, по умолчанию
`apps/server/data/balance.json`): каталог архетипов, таблица волн, параметры директора, опасности
окружения и кадр камеры. Файл не обязателен — без него сервер работает на встроенных значениях, а
старые версии файла мигрируются при загрузке. Схемы баланса не входят в версионируемый сетевой
контракт; `PROTOCOL_VERSION` растёт только когда меняется то, что видит клиент.
...
- консоль баланса `apps/admin` на `:5175` редактирует таблицу волн, каталог архетипов, параметры
  директора и кадр камеры; правки уходят в файл пресетов через защищённый `/admin/balance` и
  применяются со следующего run, идущий бой не меняется;
- display в dev-сборке открывается по `?preview=1` без сервера: панель переключает фазы и двигает
  ползунок кадра камеры, чтобы подобрать значение до сохранения в консоли;
- auto-crew демонстрации целится только по тому, что попадает в кадр камеры, а не по всей арене,
  иначе демонстрация стреляла бы по невидимым зрителю целям.
...
Развёрнутая концепция режима, включая модель урона из четырёх типов, три слоя защиты, четыре
архетипа корабля и форматы матча, вынесена в `docs/ARENA_GAME_DESIGN_DOCUMENT.md`. Модель урона того
документа относится только к арене и не меняет текущий кооперативный баланс. До отдельного OpenSpec
change этот режим не входит в текущий production scope.
