...
6. **Развитие корабля внутри матча — по образцу «Стального охотника».** Решено: не билд, собранный
   до боя, а рост по ходу — подбираешь на карте и качаешься, пока идёт матч. Значит нужны предметы в
   мире, их появление по зонам и изменение характеристик прямо в симуляции, а не между волнами:
   кампанейское дерево сюда не переносится, оно висит на архетипе корпуса, покупается кредитами и
   раздаётся карточками в антракте, а в матче нет ни волн, ни антракта, ни экономики. Шов под это
   уже есть — `ArenaShipSeat.stats` необязательный, то есть место в матче умеет начинать со своими
   числами. Туда же ложатся радиус и частота сканирования: они уже вынесены в пресет и должны стать
   ветками этого дерева. Сейчас улучшений в арене нет ни у игрока, ни у ботов — все шестнадцать
   корпусов летают на одних числах от начала матча до конца.
7. Audio pass и тонкая Capacitor-оболочка для Android TV.
8. Предпрод `stage.<домен>` за тем же прокси, с настоящим сертификатом: вторая копия контейнеров и
   выкладка со своей ветки. Нужен, когда тестировать будет не только разработчик, и обязателен для
   пушей — они требуют HTTPS и настоящий домен. До него PWA на телефонах проверяется через
   `adb reverse` на `localhost`: по голому `http://` в локальной сети service worker не
   регистрируется.
