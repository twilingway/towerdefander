...
```text
apps/display    React shell + HUD, Phaser world (Phaser lives only here), solo cockpit
...
`packages/protocol/src/index.ts` pins `PROTOCOL_VERSION` (currently 72) as a `z.literal` inside join
...
they carry their own `BALANCE_FILE_VERSION` (currently 58) with migrations in
...
A third join role, `solo`, is one connection that is both: it gets the display's `@view(1)` branch
and holds a crew seat, so one phone draws the arena and flies it. That is why the solo cockpit lives
in `apps/display` rather than in the controller — the page that already has the world grows a touch
overlay, instead of the renderer moving across a package boundary. The controller is untouched by
it; the two-device paths, solo included, still work exactly as before.
