...
describe("protocol v23 handshake and messages", () => {
  it("publishes the fixed crew and v23", () => {
  ...
  it("accepts v23 create/join and rejects v22 and unknown fields", () => {
    expect(
      displayCreateOptionsSchema.safeParse({ role: "display", protocolVersion: 23 }).success
    ).toBe(true);
    expect(
      displayCreateOptionsSchema.safeParse({ role: "display", protocolVersion: 22 }).success
    ).toBe(false);
    expect(
  ...
  it("keeps continuous role messages strict on v23 and the active run", () => {
    const envelope = {
      protocolVersion: 23,
      roomId: ROOM_ID,
      playerId: PLAYER_ID,
  ...
  it("requires the machine gun trigger on v23 pilot input", () => {
    const envelope = {
      protocolVersion: 23,
      roomId: ROOM_ID,
      playerId: PLAYER_ID,
  ...
  it("allows ready for lobby run zero and positive terminal runs", () => {
    const envelope = { protocolVersion: 23, roomId: ROOM_ID, playerId: PLAYER_ID } as const;
    ...
    expect(
      readyCommandSchema.safeParse({ ...envelope, protocolVersion: 22, runNumber: 0 }).success
    ).toBe(false);
...
describe("strict v23 room projections", () => {
...
describe("v23 latency diagnostics", () => {
  it("retains strict server probes and client pongs without client telemetry", () => {
    expect(
      serverLatencyProbeSchema.safeParse({ protocolVersion: 23, probeId: "probe-1" }).success
    ).toBe(true);
    expect(
    ...
    expect(
      serverLatencyProbeSchema.safeParse({ protocolVersion: 22, probeId: "probe-1" }).success
    ).toBe(false);
  });
