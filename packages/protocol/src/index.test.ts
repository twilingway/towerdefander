...
describe("protocol v33 handshake and messages", () => {
  it("publishes the fixed crew and v42", () => {
  ...
  it("accepts v42 create/join and rejects v41 and unknown fields", () => {
    expect(
      displayCreateOptionsSchema.safeParse({ role: "display", protocolVersion: 42, crewSize: 3 })
        .success
    ).toBe(true);
    expect(
      displayCreateOptionsSchema.safeParse({ role: "display", protocolVersion: 41, crewSize: 3 })
        .success
    ).toBe(false);
    expect(
  ...
  it("keeps continuous role messages strict on v42 and the active run", () => {
    const envelope = {
      protocolVersion: 42,
      roomId: ROOM_ID,
      playerId: PLAYER_ID,
  ...
  it("requires the machine gun trigger on v42 pilot input", () => {
    const envelope = {
      protocolVersion: 42,
      roomId: ROOM_ID,
      playerId: PLAYER_ID,
  ...
  it("allows ready for lobby run zero and positive terminal runs", () => {
    const envelope = { protocolVersion: 42, roomId: ROOM_ID, playerId: PLAYER_ID } as const;
    ...
    expect(
      readyCommandSchema.safeParse({ ...envelope, protocolVersion: 41, runNumber: 0 }).success
    ).toBe(false);
...
describe("v33 latency diagnostics", () => {
  it("retains strict server probes and client pongs without client telemetry", () => {
    ...
    expect(
      serverLatencyProbeSchema.safeParse({ protocolVersion: 41, probeId: "probe-1" }).success
    ).toBe(false);
  });
