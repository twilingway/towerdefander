...
describe("protocol v33 handshake and messages", () => {
  it("publishes the fixed crew and v50", () => {
  ...
  it("accepts v50 create/join and rejects v49 and unknown fields", () => {
    expect(
      displayCreateOptionsSchema.safeParse({ role: "display", protocolVersion: 50, crewSize: 3 })
        .success
    ).toBe(true);
    expect(
      displayCreateOptionsSchema.safeParse({ role: "display", protocolVersion: 49, crewSize: 3 })
        .success
    ).toBe(false);
    expect(
  ...
  it("takes a solo connection that is display and crew seat at once", () => {
    ...
    expect(
      joinOptionsSchema.safeParse({ role: "solo", protocolVersion: 50, playerName: "Ada" }).success
    ).toBe(true);
    ...
    expect(soloJoinOptionsSchema.safeParse({ role: "solo", protocolVersion: 50 }).success).toBe(
      false
    );
    expect(
      soloJoinOptionsSchema.safeParse({ role: "solo", protocolVersion: 49, playerName: "Ada" })
        .success
    ).toBe(false);
  });
  ...
  it("keeps continuous role messages strict on v50 and the active run", () => {
    const envelope = {
      protocolVersion: 50,
      roomId: ROOM_ID,
      playerId: PLAYER_ID,
  ...
  it("requires the machine gun trigger on v50 pilot input", () => {
    const envelope = {
      protocolVersion: 50,
      roomId: ROOM_ID,
      playerId: PLAYER_ID,
  ...
  it("allows ready for lobby run zero and positive terminal runs", () => {
    const envelope = { protocolVersion: 50, roomId: ROOM_ID, playerId: PLAYER_ID } as const;
    ...
    expect(
      readyCommandSchema.safeParse({ ...envelope, protocolVersion: 49, runNumber: 0 }).success
    ).toBe(false);
...
describe("v33 latency diagnostics", () => {
  it("retains strict server probes and client pongs without client telemetry", () => {
    expect(
      serverLatencyProbeSchema.safeParse({ protocolVersion: 50, probeId: "probe-1" }).success
    ).toBe(true);
    expect(
    ...
    expect(
      serverLatencyProbeSchema.safeParse({ protocolVersion: 49, probeId: "probe-1" }).success
    ).toBe(false);
  });
